...
describe('hapi-authorization', () => {
	...
	it('ignores routes without hapi-authorization instructions', (done) => {
		...
		server.register(plugin, {}).then(() => {
			...
			server.start().then(() => server.inject('/')).then((res) => {
	...
	it('Validates the hapi-authorization plugin option "roles" must be an array', (done) => {
		...
		server.register(plugin, {}).catch((err) => {
			expect(err).to.not.be.undefined;
			expect(err).to.be.instanceOf(Error);
			expect(err.message).to.equal('Invalid plugin options (Invalid settings) ValidationError: "roles" must be one of [array, boolean]');
			done();
		});
	});
	...
	describe('Initialize with no options', () => {
		...
		describe('ACL roles', () => {
			...
			it('returns an error when a user with unsuited role tries to access a role protected route', (done) => {
				...
				server.register(plugin, {}).then(() => {
					server.inject({method: 'GET', url: '/', auth: { strategy: 'default', credentials: {role: 'USER'} }}).then((res) => {
						internals.asyncCheck(() => {
			...
			it('Allows access to protected method for multiple authorized roles', (done) => {
				...
				server.register(plugin, {}).then(() => {
					server.inject({method: 'GET', url: '/', auth: { strategy: 'default', credentials: {role: 'ADMIN'} }}).then((res) => {
						internals.asyncCheck(() => {
			...
			it.skip('returns an error when a user with unsuited role tries to access a role protected route', (done) => {
				...
				server.register(customPluginObject, {}, (err) => {
					server.inject({method: 'GET', url: '/', auth: { strategy: 'default', credentials: {role: 'EMPLOYEE'} }}).then((res) => {
						internals.asyncCheck(() => {
							expect(res.statusCode).to.equal(403);
							expect(res.result.message).to.equal("Unauthorized");
						}, done);
			...
			it.skip('returns an error when a user with a role that is not a valid role tries to access a role protected route', (done) => {
				...
				server.register(customPluginObject, {}, (err) => {
					server.inject({method: 'GET', url: '/', auth: { strategy: 'default', credentials: {role: 'KING'} }}).then((res) => {
						internals.asyncCheck(() => {
							expect(res.statusCode).to.equal(403);
							expect(res.result.message).to.equal("Unauthorized");
						}, done);
			...
			it.skip('Allows access to protected method for multiple authorized roles', (done) => {
				...
				server.register(customPluginObject, {}, (err) => {
					server.inject({method: 'GET', url: '/', auth: { strategy: 'default', credentials: {role: 'MANAGER'} }}).then((res) => {
						internals.asyncCheck(() => {
							expect(res.payload).to.equal('Authorized');
						}, done);
					});
			...
			it('Returns an error when a user\'s role is unsuited due to hierarchy being disabled', (done) => {
				...
				server.register(plugin, {}).then(() => {
					server.inject({method: 'GET', url: '/', auth: { strategy: 'default', credentials: {role: 'OWNER'} }}).then((res) => {
						internals.asyncCheck(() => {
			...
			it('validates that the aclQuery parameter is a function', (done) => {
				...
				server.register(plugin, {}).then(() => {
					server.inject({method: 'GET', url: '/', auth: { strategy: 'default', credentials: {role: 'ADMIN'} }}).then((res) => {
						internals.asyncCheck(() => {
							expect(res.statusCode).to.equal(400)
							expect(res.result.message).to.match(/"aclQuery" must be of type function/);
						}, done);
					});
				});
			});
		...
		describe('default acl validator', () => {
			...
			it('returns error when the entity doesn\'t belong to the authenticated user', (done) => {
              ...
              return {_user: '1', name: 'Hello'};
				...
				server.register(plugin, {}).then(() => {
					server.inject({method: 'GET', url: '/', auth: { strategy: 'default', credentials: {role: 'ADMIN', _id: '2'} }}).then((res) => {
						internals.asyncCheck(() => {
							expect(res.statusCode).to.equal(403);
							expect(res.result.message).to.equal("Unauthorized");
						}, done);
			...
			it('returns the response for user with permissions', (done) => {
              ...
              return {_user: '1', name: 'Hello'};
				...
				server.register(plugin, {}).then(() => {
					server.inject({method: 'GET', url: '/', auth: { strategy: 'default', credentials: {role: 'ADMIN', _id: '1'} }}).then((res) => {
						internals.asyncCheck(() => {
							expect(res.statusCode).to.equal(200);
			...
			it('handles custom user id field', (done) => {
              ...
              return {_user: '1', name: 'Hello'};
				...
				server.register(plugin, {}).then(() => {
					server.inject({method: 'GET', url: '/', auth: { strategy: 'default', credentials: {role: 'ADMIN', myId: '1'} }}).then((res) => {
						internals.asyncCheck(() => {
							expect(res.statusCode).to.equal(200);
							expect(res.result).to.equal("Authorized");
						}, done);
		...
		describe('Joi validator with aclQuery', () => {
			...
			it('validates query parameter', (done) => {
				...
				server.register(plugin, {}).then(() => {
					server.inject({method: 'GET', url: '/?name=John Doe', auth: { strategy: 'default', credentials: {role: 'ADMIN', _id: '1'} }}).then((res) => {
						internals.asyncCheck(() => {
							expect(res.statusCode).to.equal(200);
							expect(res.result).to.equal("Authorized");
						}, done);
			...
			it('returns an error when payload parameter is missing', (done) => {
				...
				server.register(plugin, {}).then(() => {
					server.inject({method: 'POST', url: '/', payload: {}, auth: { strategy: 'default', credentials: {role: 'ADMIN', _id: '1'} }}).then((res) => {
						internals.asyncCheck(() => {
							expect(res.statusCode).to.equal(400);
			...
			it('validates payload parameter', (done) => {
				...
				server.register(plugin, {}).then(() => {
					server.inject({method: 'POST', url: '/', payload: { name: "John Doe"}, auth: { strategy: 'default', credentials: {role: 'ADMIN', _id: '1'} }}).then((res) => {
						internals.asyncCheck(() => {
							expect(res.statusCode).to.equal(200);
							expect(res.result).to.equal("Authorized");
						}, done);
...
internals.authSchema = () => {
	...
	const scheme = {
		authenticate: (request, h) => {
			return h.authenticated({ credentials: { username: "asafdav", role: 'USER'} });
		},
		payload: (request, h) => {
			return h.continue;
		},
